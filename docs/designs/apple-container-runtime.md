...
3. **Proxies bind to the vmnet gateway IP, not loopback or 0.0.0.0.** The network is created first; the proxies then bind the deterministic gateway address. Binding narrowly keeps IronCurtain's own listening surface off the LAN and off loopback (where other local processes could probe it -- same posture as today's TCP mode, where the fake sentinel key remains the application-layer guard).
