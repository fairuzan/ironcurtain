...
- `docker-manager.ts` - Docker CLI wrapper implementing `ContainerRuntime`. `getImageLabel()` reads labels for staleness detection. `buildImage()` accepts optional labels.
- `container-runtime.ts` - Runtime selection seam: `createContainerRuntime(kind)` returns the `ContainerRuntime` implementation for a `ContainerRuntimeKind` (`docker` today; `apple-container` planned, see `docs/designs/apple-container-runtime.md`). Agent-session infrastructure goes through this factory; Docker-specific host services (signal-cli, daemon orphan sweeps) call `createDockerManager()` directly.
